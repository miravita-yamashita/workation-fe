...
import {
  PostsInfoBeginnersTable,
  PostsInfoBeginnersTableTableItemKey,
  PostsInfoBeginnersTableTableItemValue,
} from "@/components/admin/static-page/detail/beginners";
import {
  BREADCRUMB_ADMIN,
  BreadcrumbComposite,
...
export default async function Page({ params }: PageProps) {
  ...
  return (
    <FormInteractionProvider>
      <Header>
        <HeaderTitle>初めての方へ一覧 For Beginners</HeaderTitle>
        <ColumnContainer className="gap-5">
          ...
          <Button size="auto" variant="remote" asChild>
            <Link href={`/admin/static-page/${id}/beginners/edit`}>更新</Link>
          </Button>
        </ColumnContainer>
      </Header>
      <Section>
        ...
        <ColumnContainer className="flex gap-5">
          <ColumnItem className="flex w-full flex-1 flex-col gap-5 rounded border p-5">
            ...
            <FieldLabelGroup className="flex flex-col gap-4">
              ...
              <PostsInfoBeginnersTable>
                <PostsInfoBeginnersTableTableItemKey className="">
                  タイトル
                </PostsInfoBeginnersTableTableItemKey>
                <PostsInfoBeginnersTableTableItemKey className="">
                  作成日
                </PostsInfoBeginnersTableTableItemKey>
                <PostsInfoBeginnersTableTableItemKey className="empty" />
                {posts.map((post, index) => (
                  <React.Fragment key={index}>
                    <PostsInfoBeginnersTableTableItemValue className="w-full whitespace-pre-line px-[.875rem] py-[.625rem] lg:px-5 lg:py-[1rem]">
                      {post.title}
                    </PostsInfoBeginnersTableTableItemValue>
                    <PostsInfoBeginnersTableTableItemValue className="w-full whitespace-pre-line px-[.875rem] py-[.625rem] lg:px-5 lg:py-[1rem]">
                      {post.created_at}

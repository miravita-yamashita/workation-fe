...
import { getAdminFaqList, MainTableData } from "@/components/admin/faq";
...
const breadcrumbData = [BREADCRUMB_ADMIN.faq.base, BREADCRUMB_ADMIN.faq.list];
...
export default async function Page({ searchParams }: PageProps) {
  const [adminFaqListResponse] = await Promise.all([
    getAdminFaqList({
      searchParams: new URLSearchParams(await searchParams).toString(),
    }),
  ]);
  const { data } = adminFaqListResponse ?? {};
  const { data: adminFaqList = [], ...paginationData } = data ?? {};
  const session = await auth();

  return (
    <DashboardActionContextProvider>
      <SectionHeader className="mx-5 justify-between py-5">
        <SectionHeaderTitle>よくある質問一覧</SectionHeaderTitle>
        <SectionHeaderContentRight>
      ...
      <SectionContent className="m-5 rounded border p-5">
        <Suspense fallback={<Loading />}>
          <MainTableData data={adminFaqList} />

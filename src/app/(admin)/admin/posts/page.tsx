...
import { getAdminPostsList, MainTableData } from "@/components/admin/posts";
...
export default async function Page({ searchParams }: PageProps) {
  const [adminPostListResponse] = await Promise.all([
    getAdminPostsList({
      searchParams: new URLSearchParams(await searchParams).toString(),
    }),
  ]);
  const { data } = adminPostListResponse ?? {};
  const { data: adminPostList = [], ...paginationData } = data ?? {};
  const recommendedArticleCount = adminPostListResponse?.recommended_count ?? 0;
  const session = await auth();

  return (
    <DashboardActionContextProvider>
      <SectionHeader className="mx-5 justify-between py-5">
        <SectionHeaderTitle>投稿一覧</SectionHeaderTitle>
        <SectionHeaderContentRight>

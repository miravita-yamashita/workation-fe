...
import {
  getAdminCustomerTestimonialList,
  MainTableData,
...
export default async function Page({ searchParams }: PageProps) {
  const [adminCustomerTestiominalsListResponse] = await Promise.all([
    getAdminCustomerTestimonialList({
      searchParams: new URLSearchParams(await searchParams).toString(),
    }),
  ]);
  const { data } = adminCustomerTestiominalsListResponse ?? {};
  const { data: adminCustomerTestimonialsList = [], ...paginationData } =
    data ?? {};
  ...
  return (
    <DashboardActionContextProvider>
      <SectionHeader className="mx-5 justify-between py-5">
        <SectionHeaderTitle>お客様の声</SectionHeaderTitle>
        <SectionHeaderContentRight>
          {session?.user?.isAdmin && (
            <TableButtonCreate className="px-[2rem] py-[.5938rem]">
              <Link
                href="/admin/customer-testimonials/new"
      ...
      <SectionContent className="m-5 rounded border p-5">
        <Suspense fallback={<Loading />}>
          <MainTableData data={adminCustomerTestimonialsList} />

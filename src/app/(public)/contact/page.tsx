...
import { BreadcrumbComposite } from "@/components/feature/breadcrumb";
import { BREADCRUMB } from "@/components/feature/breadcrumb/lib/static-values";
import Link from "next/link";

export const metadata: Metadata = {
  title: "お問い合わせフォーム（一般用）",
};
...
export default async function Page() {
  ...
  const general =
    faqPopularByGroupResponse?.data?.[FAQPopularGroup.General] ?? {};
    ...
    <>
      <BackgroundContainer className="bg-white">
        <MainBlock size="sm" className="lg:p-0">
          ...
          <PageHeader>お問い合わせ（一般の方）</PageHeader>
        </MainBlock>
      </BackgroundContainer>
          ...
          >
            <FAQAccordionPanel>
              ...
              <FAQAccordion>
                {Object.entries(general)
                  ...
                  .map(([key, { id, question, answer }]) => (
                    <FAQAccordionItem value={id} key={`${key}-${id}`}>
                      <FAQAccordionTrigger>
                        <FAQAccordionCommonContent>
                         <FAQAccordionEmphasized>
                            <span>Q</span>
              ...
              <div className="text-center">
                <FAQAccordionButton>
                  <Link
                    href={`/recruit-conform?${FAQSearchParamKey.Category}=${FAQPopularGroup.General}`}
                  >
                    もっと見る
                  </Link>
                </FAQAccordionButton>

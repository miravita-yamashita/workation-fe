...
import { getPublicTokenCookie } from "@/components/feature/public-token";
...
const getAdditionalHeaderMetaData = async () => {
  ...
  const publicToken = await getPublicTokenCookie();

  return {
    "User-Agent": headersList.get("user-agent") || "",
    "X-Forwarded-For": headersList.get("x-forwarded-for") || "",
    Authorization: `Bearer ${session?.user?.accessToken || publicToken || ""}`,
  };
};

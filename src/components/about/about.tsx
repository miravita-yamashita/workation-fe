...
export const AboutContent = ({
  items,
  isPostList = false,
}: CommonProps & { items: PostType[]; isPostList?: boolean }) => {
  const baseRedirectUrl = isPostList ? "article" : "about";
      ...
      {items?.map((content, index) => (
        <ColumnItem key={index}>
          <ImageBlock className="relative h-[11.25rem] w-full">
            <ImageItem
              src={content?.media?.featured?.[0]?.url || ImageNoPhoto?.src}
              altText=""
          ...
          <ContentAction>
            <Link
              href={`/${baseRedirectUrl}/${content?.slug}` || "#"}

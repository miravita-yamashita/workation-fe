...
import { generalFormSchema } from "./lib/form-schema";
import { z } from "zod";
...
import { useFormGeneral } from "./lib/use-form-general";
import {
  formGeneralInitialState,
  setFormGeneralValues,
  useFormGeneralStore,
} from "./lib/store";
import { useRouter, useSearchParams } from "next/navigation";

export const FormGeneral = ({ className }: CommonProps) => {
  const router = useRouter();
  const searchParams = useSearchParams();
  const storedValues = useFormGeneralStore.getState().formValues;
  ...
  const defaultValues = shouldUseStoredValues
    ? storedValues
    : formGeneralInitialState.formValues;
  // Initialize the form
  ...
  const onSubmit = async (values: z.infer<typeof generalFormSchema>) => {
    setFormGeneralValues(values);

    router.push("/contact/check");
  };
        ...
        <FieldLabelGroup>
          ...
          <FieldGrid>
            <FieldGenericCheckbox
              formHook={form}
              formInputName="inquiryTypeMedia"
              labelText="メディアに関するお問い合わせ"
            />
            <FieldGenericCheckbox
              formHook={form}
              formInputName="inquiryTypeInfringement"
              labelText="権利侵害に関するお問い合わせ"
            />
          </FieldGrid>
          <FieldGrid>
            <FieldGenericCheckbox

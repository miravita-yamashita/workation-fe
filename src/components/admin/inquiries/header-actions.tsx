...
export const HeaderActions = ({
  ...
  const handleDeleteinquiry = async () => {
    ...
    const afterSubmissionPath = "/admin/inquiries";
    try {
      setClicked(true);
      const response = await deleteInquiry(toDeleteId);
  ...
  return (
    <div className={cn("flex items-center gap-5", className)}>
      <Button
        ...
        asChild
      >
        <Link href="/admin/inquiries">戻る</Link>
      </Button>
      {!isReplied &&
        (session?.user?.isAdmin && toEdit ? (
          ...
          >
            <Link href={`/admin/inquiries/reply/${toEdit}`}>返信</Link>
          </Button>
        ) : (
          <Button
            onClick={onSubmit}

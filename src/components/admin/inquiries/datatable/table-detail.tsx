    ...
    <Table className="mb-10 mt-[.875rem] text-sm font-medium lg:mt-0">
      <TableBody className="last:border-w-1">
        {column?.map((data) => (
          ...
          >
            <TableCell className="w-[11rem] p-[.625rem] align-top text-base font-bold">
              {data?.label}
            </TableCell>
            <TableCell className="whitespace-pre-line p-[.625rem] align-top font-inter text-sm font-medium">
              {data?.value}
            </TableCell>
          </TableRow>
        ))}

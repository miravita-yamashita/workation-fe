...
export const HeaderActions = ({
  ...
  const handleDeleteJob = async () => {
    ...
    const afterSubmissionPath = "/admin/jobs";
    try {
      setClicked(true);
      const response = await deleteJob(toDeleteId);
  ...
  return (
    <div className={cn("flex items-center gap-5", className)}>
      ...
      {session?.user?.isAdmin && toEdit ? (
        ...
        >
          <Link href={`/admin/jobs/edit/${toEdit}`}> 編集する</Link>
        </Button>
      ) : (
        <Button
          onClick={onSubmit}

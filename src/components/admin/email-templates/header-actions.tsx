...
import { deleteEmailTemplate } from "./lib";
...
export const HeaderActions = ({
  ...
  const handleDelete = async () => {
    ...
    const afterSubmissionPath = "/admin/email-templates";
    try {
      setClicked(true);
      const response = await deleteEmailTemplate(toDeleteId);
  ...
  return (
    <div className={cn("flex items-center gap-5", className)}>
      <Button
        ...
        asChild
      >
        <Link href="/admin/email-templates">戻る</Link>
      </Button>
      {session?.user?.isAdmin && toEdit ? (
        ...
        >
          <Link href={`/admin/email-templates/edit/${toEdit}`}>編集</Link>
        </Button>
      ) : (
        <Button
          onClick={onSubmit}

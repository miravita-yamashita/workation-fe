...
import { z } from "zod";
import { formAdminStaticAboutSchema } from "./lib";
import {
...
import { useFormStaticAbout } from "./use-form-static-about";
...
export const FormStaticAbout = ({
  className = "",
  id,
  info,
  postsNameList,
}: FormStaticAboutProps) => {
  const { positions } = info || {};
  const jobNumbers =
    positions?.slice(0, 3)?.map((position) => position?.job_number) || [];

  const form = useForm<z.infer<typeof formAdminStaticAboutSchema>>({
    resolver: zodResolver(formAdminStaticAboutSchema),
    mode: "onBlur",

...
export const HeaderActions = ({
  ...
  const handleDeleteJob = async () => {
    ...
    const afterSubmissionPath = "/admin/faq";
    try {
      setClicked(true);
      const response = await deleteFaq(toDeleteId);
  ...
  return (
    <div className={cn("flex items-center gap-5", className)}>
      ...
      {session?.user?.isAdmin && toEdit ? (
        ...
        >
          <Link href={`/admin/faq/edit/${toEdit}`}> 編集する</Link>
        </Button>
      ) : (
        <Button
          onClick={onSubmit}

...
import { useFormNurseStore } from "@/components/feature/form";
import { submitFormNurse } from "./lib/actions";
...
export const CheckDetails = () => {
  ...
  const formValues = useFormNurseStore.getState().formValues;
  const {
    lastName,
    firstName,
    sei,
  ...
  const hasStoredValues = useFormNurseStore.getState().hasStoredValues;

  const handleSubmit = async () => {
    setIsLoading(true);

    try {
      const response = await submitFormNurse(
        InquiryFromType.Nurse,
        formValues,
      );
      ...
      router.push("/contact-nurse/thanks");
    ...
    return (
      <div>
        ...
        <Button
          ...
          asChild
        >
          <Link href="/contact-nurse">内容を変更する</Link>
        </Button>
      </div>
    );
  }
    ...
    <DetailPanel>
      ...
      <DetailPanelAction>
        <Button
          ...
          asChild
        >
          <Link href="/contact-nurse?persist=true">内容を変更する</Link>
        </Button>
        <Button
          disabled={isLoading}
          variant="form"
